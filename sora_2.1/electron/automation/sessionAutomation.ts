...
async function safeDisconnect(browser: Browser | null) {
    if (browser) {
        try { await browser.disconnect(); } catch {}
    }
}

const resolveProfileForContext = async (sessionName: string, sessionId: string): Promise<ChromeProfile> => {
  ...
  try {
    const handle = await page.evaluateHandle((ta: any) => {
      ...
      for (const btn of buttons) {
        ...
        if (Math.abs(btnCenterY - taCenterY) > 150) continue; 
...
export async function runPromptsAdapter(
    ...
    try {
        ...
        if (activePrompts.length === 0) {
            throw new Error(`No prompts found in ${path.basename(paths.promptsFile)}.`);
        }
