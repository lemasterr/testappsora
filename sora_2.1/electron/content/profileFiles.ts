...
import { ASSETS_DIR_NAME } from '../config/constants';
...
async function ensurePromptsDir(sessionsRoot: string): Promise<string> {
  const dir = path.join(sessionsRoot, ASSETS_DIR_NAME);
  await ensureDir(dir);
  return dir;
}
...
export async function saveProfileFiles(profileName: string | null, files: SessionFiles): Promise<{ ok: boolean; error?: string }> {
  ...
  try {
    ...
    logInfo('profileFiles', `Saved files for profile ${normalizedName} to ${ASSETS_DIR_NAME}`);
    return { ok: true };
  } catch (error) {
