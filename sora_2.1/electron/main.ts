...
import { runDownloads } from './automation/downloader';
import { runPipeline, cancelPipeline } from './automation/pipeline';
import { stopTask, cancelAllTasks } from './automation/taskRegistry';
import { listBlurProfiles, saveBlurProfile, deleteBlurProfile, blurVideo, type BlurZone } from './video/ffmpegBlur';
...
import { startPythonServer, stopPythonServer, pythonGetStats, pythonGetTopSessions } from './integrations/pythonClient';
...
reg('pipeline:skip', async () => { cancelAllTasks(); return { ok: true }; });

// --- INSPECTOR ---
reg('inspector:start', async (sessionId) => startInspectorForSession(sessionId));
reg('inspector:poll', async (sessionId) => getLastSelectorForSession(sessionId));
...
function rectsToZones(rects: any[] = []): BlurZone[] {
  return rects.map((r) => ({ x: r.x, y: r.y, w: r.w ?? r.width, h: r.h ?? r.height, mode: r.mode, blur_strength: r.blur_strength, band: r.band } as any));
}
...
reg('analytics:getDailyStats', async (d) => {
    try {
        const stats = await pythonGetStats(d || 14);
...
reg('analytics:getTopSessions', async (l) => {
    try {
        const sessions = await pythonGetTopSessions(l || 5);
        return sessions.map((s: any) => ({
            sessionId: s.session_id || 'unknown',
            downloaded: s.download_count || 0
        }));
    } catch (e) {
        return [];
    }
