...
export async function ensureCloneSeededFromProfile(
  ...
  // 3. Remove SingletonLock to prevent "Profile in use" crashes
  const lockFile = path.join(targetDefaultDir, 'SingletonLock');
  const rootLockFile = path.join(cloneDir, 'SingletonLock');
...
export async function resolveProfileLaunchTarget(
  ...
  // ChromeLauncher should ideally ignore this arg if we structure the folder correctly,
  // but for consistency we call it 'Default'.
