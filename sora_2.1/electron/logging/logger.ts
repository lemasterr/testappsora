...
const logBuffer: LogEntry[] = []; // For disk flush
const uiHistoryBuffer: LogEntry[] = []; // For UI display
const MAX_UI_HISTORY = 1000;
...
function emit(level: LogEntry['level'], source: string, msg: string): void {
  ...
  // Add to UI history
  uiHistoryBuffer.push(entry);
  if (uiHistoryBuffer.length > MAX_UI_HISTORY) {
      uiHistoryBuffer.shift();
  }
