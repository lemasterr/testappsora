...
const StepNode = ({ data, selected, id }: any) => {
  ...
  // Validation Check
  const isValid = !isGenericPrompt || (data.promptInputSelectorId && data.submitSelectorId);
          ...
          <div className="mt-1 pt-2 border-t border-zinc-800 space-y-2">
              ...
              {(data.action === 'type' || data.action === 'navigate' || data.action === 'wait') && (
                  <input 
                    ...
                    placeholder={data.action === 'wait' ? 'or ms (e.g. 2000)' : data.action === 'navigate' ? 'https://...' : 'Value to type...'}
                    value={data.value || ''} 
...
export const AutomatorPage: React.FC = () => {
  const { sessions, automator, setAutomatorNodes, setAutomatorEdges, saveAutomatorPreset, loadAutomatorPreset, deleteAutomatorPreset, workflowStatus } = useAppStore();
  ...
  return (
    ...
    <div className="flex h-[calc(100vh-8rem)] gap-6">
      <div className="flex-1 flex flex-col min-w-0 gap-4">
        <div className="flex items-center justify-between bg-gradient-to-r from-zinc-900 to-black p-4 rounded-2xl border border-zinc-800 shadow-lg">
            ...
            <div className="flex items-center gap-2 px-4 border-x border-zinc-800">
                <div className="flex items-center gap-1">
                    <input type="text" className="input-field py-1.5 text-xs w-32 bg-zinc-900" placeholder="New Preset" value={newPresetName} onChange={(e) => setNewPresetName(e.target.value)}/>
                    <button onClick={handleSavePreset} className="p-1.5 rounded bg-indigo-600/20 text-indigo-400 border border-indigo-600/30"><Icons.Check className="w-4 h-4" /></button>
                </div>
                <div className="w-px h-6 bg-zinc-800 mx-2"/>
                <div className="flex items-center gap-1">
                    <select className="bg-zinc-900 border border-zinc-700 text-zinc-300 text-xs rounded px-2 py-1.5 w-32 focus:outline-none" onChange={handleLoadPreset} value={selectedPresetId}>
            ...
            <div className="flex items-center gap-3">
                <button onClick={deleteSelected} disabled={!hasSelection} className="btn-secondary text-rose-400 border-rose-900/30 disabled:opacity-30"><Icons.Trash className="w-3 h-3 mr-2" /> Delete</button>
                <div className="w-px h-6 bg-zinc-800 mx-1"/>
                <button onClick={clearGraph} className="btn-secondary text-zinc-400">Clear</button>
                <div className="w-px h-6 bg-zinc-800 mx-1"/>
                ...
                {status === 'running' && (<><button onClick={skipStep} className="btn-secondary py-2 px-3 text-amber-400 border-amber-500/30"><Icons.ChevronRight className="w-4 h-4"/></button><button onClick={stop} className="btn-danger py-2 px-3"><Icons.Stop className="w-4 h-4"/></button></>)}
            </div>
        </div>

        <div className="flex-1 rounded-2xl border border-zinc-800 bg-[#0c0c0e] shadow-inner relative overflow-hidden group" ref={reactFlowWrapper}>
      ...
      <div className="w-64 flex flex-col gap-4 bg-zinc-900/30 p-4 rounded-2xl border border-zinc-800/50 backdrop-blur-sm">
