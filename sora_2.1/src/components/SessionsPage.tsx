  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] gap-6">
      ...
      <div className="flex-1 flex flex-col bg-[#0c0c0e] rounded-2xl border border-zinc-800 shadow-xl overflow-hidden">
        {/* Toolbar */}
        <div className="h-16 border-b border-zinc-800 bg-zinc-900/30 backdrop-blur flex items-center justify-between px-6 shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            {!form.id ? (
                ...
                <div className="flex items-center text-xs font-mono">
                    {saving ? <span className="text-blue-400 animate-pulse">Auto-saving...</span> : <span className="text-zinc-500">{message}</span>}
                </div>
            )}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              ...
              <section className="space-y-4">
                ...
                <div className="bg-zinc-900/20 p-5 rounded-xl border border-zinc-800/50 space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <Input label="Batch Limit" type="number" value={form.maxPromptsPerRun || ''} onChange={(e: any) => handleChange('maxPromptsPerRun', +e.target.value)} hint="Prompts/run (Default: 2)" />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <Input label="Post-Run Delay (ms)" type="number" value={form.postLastPromptDelayMs || ''} onChange={(e: any) => handleChange('postLastPromptDelayMs', +e.target.value)} hint="Wait time after batch (Default: 120000)" />
                  </div>
                </div>
              </section>
            </div>
            ...
            <div className="space-y-8">
              ...
              <div className="pt-6 border-t border-zinc-800/50">
                ...
                {message && form.id && !saving && <div className="text-center text-xs text-emerald-400 mt-3 font-medium animate-pulse">{message}</div>}
              </div>
            </div>
          </div>
