...
export const GalleryPage: React.FC = () => {
  ...
  const handleDelete = async (path: string) => {
      ...
      const res = await window.electronAPI.gallery.delete(path);
      if (res.ok) {
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex flex-col gap-6">
        {/* Header */}
        <div className="flex items-center justify-between p-6 rounded-3xl bg-gradient-to-br from-indigo-900/20 via-zinc-900/50 to-black border border-white/10 shadow-xl shrink-0">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                    {filtered.map((video) => (
                        <VideoCard 
                            key={video.path} 
                            video={video} 
                            playing={playing === video.path} 
                            setPlaying={setPlaying} 
                            onDelete={() => handleDelete(video.path)}
                        />
                    ))}
                </div>
            )}
...
const VideoCard: React.FC<VideoCardProps> = ({ video, playing, setPlaying, onDelete }) => {
    ...
    return (
        ...
        >
            ...
            <button 
                ...
                className="absolute top-2 right-2 p-1.5 bg-black/60 hover:bg-rose-500/90 rounded-lg text-white opacity-0 group-hover:opacity-100 transition-all z-20 backdrop-blur-sm border border-white/10"
                title="Delete Video"
            >
            ...
            {/* Tags */}
            {video.type === 'clean' && (
                <div className="absolute top-2 left-2 bg-emerald-500/90 text-black text-[9px] font-bold px-1.5 py-0.5 rounded shadow-sm z-10 pointer-events-none">CLEAN</div>
            )}
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent opacity-60 group-hover:opacity-100 transition-opacity p-3 flex flex-col justify-end pointer-events-none">
                <div className="text-xs font-bold text-white truncate drop-shadow-md">{video.name}</div>
                <div className="flex justify-between items-center mt-1">
                    <span className="text-[10px] text-indigo-300 bg-indigo-500/20 px-1.5 py-0.5 rounded border border-indigo-500/30 truncate max-w-[80px] backdrop-blur-md">{video.sessionName}</span>
                    <span className="text-[9px] text-zinc-400 font-mono">{(video.size / 1024 / 1024).toFixed(1)} MB</span>
                </div>
            </div>
        </div>
    );
