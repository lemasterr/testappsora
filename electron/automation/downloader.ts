...
// Fixed import path (removed .js extension for better resolution compatibility)
...
async function preparePage(browser: Browser, downloadDir: string): Promise<Page> {
  ...
  if (!page.url().includes('sora.chatgpt.com')) {
    try {
        await page.goto(pages.draftsUrl, { waitUntil: 'networkidle2', timeout: 60000 });
    } catch (e) {
        logError('Downloader', `Page load timeout/error: ${(e as Error).message}`);
    }
  }
...
export async function runDownloads(
  ...
  try {
    ...
    const configMax = session.maxVideos || 0;
    // If both are 0, use a high safety cap (10000) for "unlimited"
    const targetTotal = explicitCap > 0 ? explicitCap : (configMax > 0 ? configMax : 10000);

    logInfo('Downloader', `Session ${session.name}: Found ${mp4Count} existing videos. Target total: ${targetTotal}.`);

    // OPTIMIZATION: If we already met the target, DON'T launch Chrome.
    ...
    if (page) {
        try {
            await waitForVisible(page, selectors.cardItem);
        } catch {
            logError('Downloader', 'Failed to find initial cards. Is the page loaded?');
        }
